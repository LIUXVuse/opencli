...
export async function buildManifest(): Promise<BuildManifestResult> {
  const main = await scanClisDir(CLIS_DIR);
  ...
  if (!fs.existsSync(COMPILED_CLIS_DIR)) return main;

  const mainKeys = new Set(main.entries.map(e => `${e.site}/${e.name}`));
  const compiledEntries: ManifestEntry[] = [];
  const compiledFailures: ManifestImportError[] = [];
  ...
  for (const site of fs.readdirSync(COMPILED_CLIS_DIR)) {
    ...
    for (const file of fs.readdirSync(siteDir)) {
      if (file.endsWith('.js') && !file.endsWith('.d.js') && !file.endsWith('.test.js') && file !== 'index.js') {
        const filePath = path.join(siteDir, file);
        try {
          const entries = await loadManifestEntries(filePath, site, undefined, COMPILED_CLIS_DIR);
          for (const entry of entries) {
            const key = `${entry.site}/${entry.name}`;
            if (!mainKeys.has(key)) {
              // Rewrite modulePath relative to CLIS_DIR for runtime resolution
              const relPath = path.relative(CLIS_DIR, filePath).replace(/\\/g, '/');
              entry.modulePath = relPath;
              if (entry.sourceFile) entry.sourceFile = relPath;
              compiledEntries.push(entry);
              mainKeys.add(key);
            }
          }
        } catch (err) {
          if (err instanceof ManifestImportError) { compiledFailures.push(err); continue; }
          throw err;
        }
      }
    }
  ...
  const allEntries = [...main.entries, ...compiledEntries]
    .sort((a, b) => a.site.localeCompare(b.site) || a.name.localeCompare(b.name));

  return { entries: allEntries, failures: [...main.failures, ...compiledFailures] };
}
